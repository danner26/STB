...
(function () {
  ...
  describe('Articles Controller Tests', function () {
    ...
    it('$scope.find() should create an array with at least one article object fetched from XHR', inject(function (Articles) {
      // Create a sample listings array that includes the new article
      var sampleArticles = [mockArticle];

      // Set GET response
      $httpBackend.expectGET('api/listings').respond(sampleArticles);
    ...
    describe('$scope.create()', function () {
      ...
      it('should send a POST request with the form input values and then locate to new object URL', inject(function (Articles) {
        // Set POST response
        $httpBackend.expectPOST('api/listings', sampleArticlePostData).respond(mockArticle);
        ...
        expect($location.path.calls.mostRecent().args[0]).toBe('listings/' + mockArticle._id);
      }));

      it('should set scope.error if save error', function () {
        var errorMessage = 'this is an error message';
        $httpBackend.expectPOST('api/listings', sampleArticlePostData).respond(400, {
    ...
    describe('$scope.update()', function () {
      ...
      it('should update a valid article', inject(function (Articles) {
        ...
        expect($location.path()).toBe('/listings/' + mockArticle._id);
      }));
    ...
    describe('$scope.remove(article)', function () {
      beforeEach(function () {
        // Create new listings array and include the article
    ...
    describe('scope.remove()', function () {
      ...
      it('should redirect to listings', function () {
        expect($location.path).toHaveBeenCalledWith('listings');
      });
    });
  });
}());
